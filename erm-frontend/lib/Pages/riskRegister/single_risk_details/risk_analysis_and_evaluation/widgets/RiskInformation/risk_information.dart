...
class _RiskInformationState extends State<RiskInformation> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: SingleRiskRepository().getRiskById(),
      builder: (context,snapshot) {
         if(snapshot.connectionState == ConnectionState.waiting){
           return  Expanded(child: ShimmerBoxWidget(height: MediaQuery.of(context).size.height,));
        ...
        return Padding(
          ...
          Container(
                  ...
                  child: Icon(
                    (showEditing == false)?Icons.edit:Icons.close,
                ...
                Stack(
                  children: [
                    ...
                    (showEditing == false)?
                    Positioned(child: RiskInfomationDetails(data:snapshot.data))
    ...
    return Container(                      
      ...
      child: Column(
        children: [
          ...
          const Text('Risk Priority',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500),),
      ...
      const SizedBox(height: 14,),
          const Text('Risk Category',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500),),
          const SelectPrioRity(),
          const SizedBox(height: 14,),
          const Text('Risk Division',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500),),
          const SelectPrioRity(),
          const SizedBox(height: 14,),
          const Text('Strategic Objective',style: TextStyle(fontSize: 16,fontWeight: FontWeight.w500),),
          const SelectPrioRity(),
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      child: Column(
          ...
          children: [
            ...
            Container(
              ...
              height: 15,
              decoration: BoxDecoration(
                  color: ( data['priority'] =='1')?Colors.red:( data['priority'] =='2')?Colors.amber:( data['priority'] =='3')?Colors.blue:Colors.green,
     ...
     RiskInfoSubHeadingAndText(
      ...
      text: data['risk_description']
    ),
     RiskInfoSubHeadingAndText(
      subheading: 'RISK OWNER',
      text: "${data['risk_owner']['firstname']} ${data['risk_owner']['lastname']}",
    ),
    RiskInfoSubHeadingAndText(
      subheading: 'RISK CATEGORY',
      text: data['risk_category'],
    ),
     RiskInfoSubHeadingAndText(
      subheading: 'DATE IDENTIFIED',
      text: DateFormat('E dd MMM yyyy').format(DateTime.parse(data['created_at']))
    ),
     RiskInfoSubHeadingAndText(
      subheading: 'DIVISION',
      text: data['risk_division'],
    ),
    const RiskInfoSubHeadingAndText(
